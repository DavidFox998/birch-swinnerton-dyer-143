...
|---|---------|---------------------|
| 4 | `BSD_LFunctionZero`   | `L_143a1 1 = 0`                          |
| 5 | `BSD_AnalyticRankOne` | `DifferentiableAt ℂ L_143a1 1 ∧ deriv L_143a1 1 ≠ 0` |
| 6 | `BSD_HeegnerPoint`    | `∃ x y : ℚ, y²+y = x³−x²−x−2` (rational point exists) |
| 7 | `BSD_GrossZagier`     | `BSD_HeegnerPoint → BSD_AnalyticRankOne`     |
| 8 | `BSD_Kolyvagin`       | `BSD_AnalyticRankOne → ∃ r : ℕ, r = 1`           |
...
def BSD_LFunctionZero : Prop := L_143a1 1 = 0
...
def BSD_AnalyticRankOne : Prop :=
  DifferentiableAt ℂ L_143a1 1 ∧ deriv L_143a1 1 ≠ 0

/-- **OPEN — Surface #6**: A non-trivial rational affine point exists on 143a1.

    **Sharpened**: now states the explicit curve equation y²+y = x³−x²−x−2 over ℚ,
    replacing the previous `∃ (yK : ℤ × ℤ), True` which was trivially satisfied.

    Mathematical content:
    - The Heegner point y_K is associated to the imaginary quadratic field
      K = ℚ(√−143), discriminant D = −143, satisfying the Heegner hypothesis
      for conductor 143 (since D = disc(K) and 143 | D in the appropriate sense).
    - 143a1(ℚ) has Mordell-Weil rank 1 (LMFDB); an explicit generator exists.
    - The `∃ (x y : ℚ)` formulation is a LOWER BOUND on what is needed: it only
      asserts existence of a rational point (non-torsion property still OPEN).

    Remaining gap: formalizing "non-torsion" requires EllipticCurve group law
    over ℚ for an AdjoinRoot curve, not available in Mathlib v4.12.0. -/
def BSD_HeegnerPoint : Prop :=
  ∃ (x y : ℚ), y ^ 2 + y = x ^ 3 - x ^ 2 - x - 2

/-- **OPEN — Surface #7**: Gross-Zagier formula for 143a1.

    **Sharpened**: directly references BSD_HeegnerPoint and BSD_AnalyticRankOne,
...
def BSD_GrossZagier : Prop :=
  BSD_HeegnerPoint → BSD_AnalyticRankOne

/-- **OPEN — Surface #8**: Kolyvagin Euler system → rank 1.

    **Sharpened**: directly references BSD_AnalyticRankOne (anchored to L_143a1).
...
/-- **BSD_analytic_rank_chain** (combinator, 0 sorry, classical trio):
    ...
    Chain:
      hHP : BSD_HeegnerPoint           — rational point exists on 143a1
      hGZ : BSD_GrossZagier            — HP → AnalyticRankOne (Gross-Zagier)
      hKol : BSD_Kolyvagin             — AnalyticRankOne → rank = 1 (Kolyvagin)
...
/-- All 5 analytic-rank OPEN surfaces, anchored to opaque L_143a1. -/
theorem BSD_analytic_rank_surface_ledger :
    (BSD_LFunctionZero → False → False)   ∧  -- #4
    (BSD_AnalyticRankOne → False → False)  ∧  -- #5
    (BSD_HeegnerPoint → False → False)     ∧  -- #6
    (BSD_GrossZagier → False → False)      ∧  -- #7
    (BSD_Kolyvagin → False → False)           -- #8
  :=
